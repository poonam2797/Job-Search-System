...
public class EmployerControllerTest {
	...
	public void whenValidInput_thenRegisterEmployer() throws IOException, Exception {
		...
		mvc.perform(post("/springfox/api/jss/employers/").contentType(MediaType.APPLICATION_JSON).content(JsonUtil.toJson(infosys)));
		mvc.perform(post("/springfox/api/jss/employers/").contentType(MediaType.APPLICATION_JSON).content(JsonUtil.toJson(capgemini)));

		List<Employer> found = employerSpringDatadao.findAll();
		assertThat(found).extracting(Employer::getOrganizationName).containsOnly("Infosys","Capgemini");
	...
	public void whenValidInput_thenPostAJob() throws IOException, Exception {
		...
		mvc.perform(post("/springfox/api/jss/employers/postAJob/1").contentType(MediaType.APPLICATION_JSON).content(JsonUtil.toJson(j1)));
		mvc.perform(post("/springfox/api/jss/employers/postAJob/2").contentType(MediaType.APPLICATION_JSON).content(JsonUtil.toJson(j2)));
    ...
    public void testDeleteJob() {
         int id = 1;
         Job job = restTemplate.getForObject("/springfox/api/jss/employers/delete/"+id, Job.class);
         restTemplate.delete("/springfox/api/jss/employers/delete/"+id);
         try {
              job = restTemplate.getForObject("/springfox/api/jss/employers/delete/"+id, Job.class);
